...
Availability: Quip is freely available under the 3-clause BSD license from
...
cutoff, and the read is encoded as a position within the contig set. This
...
arbitrary count can be associated with each element \citep{Fan2000}. A very
similar data structure, the CountMin Sketch, is described by Carmode and Muthukrishnan (TODO:
citation), which was arrived at independently of the prior Bloom filter research.
\citet{Bonomi2006} subsequently described the d-left counting Bloom filter (dlCBF),
a refinement of the counting Bloom filter requiring significantly less space to
achieve the same false positive rate.
...
Memory efficient assembly has been a goal of particular interest and is a
topic of ongoing research. Conway and Bromage (TODO: citation) have developed
a more efficient means of representing de Bruijn graphs using sparse bitmaps
compressed with Elias-Fano encoding. The String Graph Assembler described by
Simpson, et al, (TODO: citation) takes an entirely different approach, relying
on a the FM-index (TODO: citation) to build compact representation of the set
of short reads from which contigs are generating by searching for overlaps.
Both of these algorithms sacrifice time-efficiency for their promising
increases in space-efficiency, reporting significantly longer run times than
traditional assemblers (e.g. both were compared to ABySS \citep{Simpson2011}).

Our work is a significant departure in that we abandon exact representations
and rely on a probabilistic data structure, an idea that has only very
recently been breeched. Counting $k$-mers efficiently with the help of
...
Concurrently with our work, \citet{Pell2011} explore the use of probabilistic
...
discussion to data compression. \citet{Pell2011} on the other hand provide a
...
unmapped-reads}), it is unable to store the original read IDs. Since all other
programs evaluated were entirely lossless, measuring compressed file sizes is
not an entirely fair comparison. Cramtools is put at some advantage simply by
excluding identifiers. However, as the only other available reference-based
...
resulting in a large reduction in compressed file size. Metagenomic sequencing
