...
for fn in glob("*.comp.csv"):
    prog = re.match(r"(.+)\.comp\.csv", fn).group(1)

    for line in open(fn):
        row = line.strip().split(",")

        samp = re.match(r"(\d+)(_\d+)?\.fastq", row[0]).group(1)
        time = row[1]
